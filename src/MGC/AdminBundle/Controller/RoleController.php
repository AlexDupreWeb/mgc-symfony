...
class RoleController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $queryBuilder = $this->getDoctrine()->getRepository("AdminBundle:Role")->createQueryBuilder('r');
        ...
        //Count All Roles
        $rolesCount = $this->getDoctrine()->getRepository("AdminBundle:Role")->createQueryBuilder('c')->select('COUNT(c.id)')->getQuery()->getSingleScalarResult();
        $rolesEnabledCount = $this->getDoctrine()->getRepository("AdminBundle:Role")->createQueryBuilder('c')->select('COUNT(c.id)')->where("c.active='1'")->getQuery()->getSingleScalarResult();
        $rolesDisabledCount = $this->getDoctrine()->getRepository("AdminBundle:Role")->createQueryBuilder('c')->select('COUNT(c.id)')->where("c.active='0'")->getQuery()->getSingleScalarResult();
    ...
    {
        return $this->render('AdminBundle:Role:add.html.twig');
    }
}
